 ...
 * 功能: 好友面板
 **/
public class FriendPanel extends BaseCommand {
    public FriendPanel(Client client, Msg msg, String key) {
        super(client, msg, key);
    }

    @Override
    public void execute() {
        getWinMsg().setDesc("我的好友");
        getWinMsg().setCol(2);
        List<Choice> choice = new ArrayList<>();
        List<Relation> relations = RelationCache.relationMap.get(getPlayer().getAccount());
        if (relations.size() > 0) {
            for (Relation relation : relations) {
                if (relation.getType() == Enum.relation.好友) {
                    try {
                        ...
                        Choice preAdd = Choice.createChoice(
                                ...
                                "friendTalk",targetPlayer==null?null:targetPlayer.getKey(), player.getAccount(), open
                        );
