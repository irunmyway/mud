...
layui.define('layim', (exports) => {
  ...
  layim.config({
    ...
    msgbox: setter.views + 'layim/msgbox.html', //消息盒子页面地址，若不开启，剔除该项即可
    find: setter.views + 'layim/find.html', //发现页面地址，若不开启，剔除该项即可
    chatLog: setter.views + 'layim/chatlog.html', //聊天记录页面地址，若不开启，剔除该项即可
  })
  //监听在线状态的切换事件
  ...
  layim.on('sendMessage', (data) => {
    ...
    setTimeout(() => {
      ...
      if (To.type === 'group') {
        ...
        layim.setChatStatus('<span class="layui-text-red">在线</span>')
      }
  ...
  let event = {
    ...
    mobile() {
      let device = layui.device(),
        mobileHome = setter.views + 'layim/mobile.html'
