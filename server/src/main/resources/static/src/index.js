  ...
  .define(['setter', 'admin'], (exports) => {
    let $ = layui.$,
      ...
      renderPage = () => {
        ...
        if (!path.length) path = ['']
        // 如果最后一项为空字符，则读取默认文件
        if (path[path.length - 1] === '') path[path.length - 1] = setter.entry
        ...
        let reset = (type) => {
          if (renderPage.haveInit) {
            $('.layui-layer').each(function () {
              ...
              if (!othis.hasClass('layui-layim')) layer.close(index)
            })
          }
        ...
        view()
          ...
          .then(function (res) {
            ...
            // 定位当前tabs
            element.tabChange(FILTER_TAB_TABS, pathURL)
            admin.tabsBodyChange(tabsPage.index)
          })
          .done(() => {
    ...
    // 对外输出
    exports('index', { render: renderPage })
  })
